...
class sensor:
   ...
   """
   def __init__(self, sbc, chip, gpio, model=DHTAUTO, callback=None):
      """
      Instantiate with the sbc, gpiochip, and the GPIO connected
      ...
      self._cb = sbc.callback(chip, gpio, rgpio.RISING_EDGE, self._rising_edge)
   ...
   def _trigger(self):
      self._sbc.gpio_claim_output(self._chip, self._gpio, 0)
      ...
      self._bits = 0
      self._code = 0
      self._sbc.gpio_claim_alert(self._chip, self._gpio, rgpio.RISING_EDGE)
